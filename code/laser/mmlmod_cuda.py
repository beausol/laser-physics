...
if single_precision:
    ftype = np.dtype('float32')
    ctype = np.dtype('complex64')
    def number(x):
        return np.float32(x)
else:
    ftype = np.dtype('float64')
...
class LaserResonatorParameters(object):
    ...
    def __init__(self, r_1, r_2, loss_db=0.0):
        ...
        self.r_1 = number(r_1)
        self.r_2 = number(r_2)
        self._loss_db = number(loss_db)
        
        gamma = np.sqrt(self.r_1 * self.r_2 * number(10)**(-self._loss_db/number(10)))
        self.tau_pho = -number(0.5)/np.log(np.abs(gamma))
...
class LaserMaterialParametersFLL(LaserMaterialParameters):
    ...
    def __init__(self, tau_par, gbar_0, alpha = 0.0, i_sat = 1.0):
        ...
        i_sat : numpy.ndarray.float64
            Saturation intensity adjustment; default = 1.0
        '''
        self.tau_par = number(tau_par)
        self.gbar_0 = number(gbar_0)
        self.alpha = number(alpha)
        self.i_sat = number(i_sat)
...
class MultiModeLaserModel(object):
    ...
    def _simplot(self, usepolar, usetex, uselog, show, dirpath, job_index, format_string):
        ...
        dphidt = np.gradient(phi_out, t_rt)
    ...
    def integrate(self, t_max, max_step=np.inf, eq0=None, method='RK23',
        ...
        max_step : float
           Maximum integration step size, expressed in units of the group round-trip
           propagation time; default = numpy.inf
        ...
        if eq0 is None:
            ...
            assert eq0.dtype == np.dtype('complex128'), "eq0 should be a numpy array with dtype = complex128."
        ...
        self._simplot(usepolar, usetex, uselog, show, dirpath, job_index, format_string)

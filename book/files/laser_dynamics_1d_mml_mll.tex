...
Referring to \eqn{mml_1d_deq_dt_final}, when $\dot{E}_q(t) = 0$, we find that $F_q$ must also satisfy the expression
\begin{equation}
    F_q = B_q\, E_q\, ,
\end{equation}
where
\begin{equation}
    B_q \equiv \frac{1}{2\, \tau_\lambda} \left(1 + i\, \Omega_q\right) - i\, \delta D_q\, .
\end{equation}
Therefore,
\begin{equation}
...
% The first term on the \rhs of \eqn{mll_1d_kappa_swl} and \eqn{mll_1d_kappa_swl_smpl} is simply the nonlinear coupling for the unidirectional ring laser. The second term arises from cross-saturation \emph{neglecting} interference between the counterpropagating fields, while the third term adds these interference effects. Note that \eqn{mll_1d_kappa_swl_smpl} predicts that $\kappa_{qqq} = 3$, which is the saturation constant appropriate for weak fields as described toward the end of \sct{laser_statics_1d_shb}.
 ...
 \subsection{Numerics}
Let's assume that in all cases of practical interest the transverse coherence time $\tau_\perp$ (which has been scaled by the group round-trip time $\tau_g$) is small enough that we can ignore the corresponding differential operators on the \rhs of \eqn{mll_swl_fqt}, giving
...
B_{m n} \equiv \half\, \left( \mathcal{L}_m + \mathcal{L}^\ast_n \right)\, .
 \end{equation}
From \eqn{mll_diff_oper}, we note that
 \begin{equation} %\label{eqn:mll_diff_oper}

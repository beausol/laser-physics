...
\begin{equation}
  \frac{2\, R_q}{\Lq} \longrightarrow \frac{2\, R_q}{\Lq} + i \left[ \frac{1}{\Lq\, \tau_\lambda} - \frac{2\, R_q}{\mathcal{A}} \right] \Omega_q\, \epsilon
\end{equation}

\begin{equation}
  C_{m n} \longrightarrow \sum_{l = 0}^\infty C_{m n} \left(C_{m n} - 1\right)^l \epsilon^l
\end{equation}

\begin{equation}
  B_{m n}\, C_{m n} \longrightarrow B_{m n}\, C_{m n} + i \left[ \frac{\Omega_m - \Omega_n}{2\, \tau_\lambda} + C_{m n} \left(C_{m n} - 1\right) \right] \epsilon
\end{equation}
...
valid for $q \in \{0, \dots, 2\, p_\textrm{max}\}$.
 ...
 \subsection{Passive Temporal Mode-Locking with a Saturable Absorber}
 \subsection{Passive Frequency Mode-Locking}
